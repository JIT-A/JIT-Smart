...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=8, pin_memory=True, worker_init_fn=worker_init_fn)
...
def evaluate(args, model, tokenizer, eval_when_training=False, eval_dataset=None):
    ...
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=8, pin_memory=True, worker_init_fn=worker_init_fn)
...
def test(args, model, tokenizer, best_threshold=0.5, eval_dataset=None):
    ...
    with torch.no_grad():
        for batch in tqdm(eval_dataloader):
            ...
            loss, logit, attn_weights, line_dl_score = model(inputs_ids, attn_masks, manual_features, labels, line_ids, line_label, output_attentions=True)
            # break
    ...
    for example, pred, prob, attn, line_sc in zip(eval_dataset.examples, y_preds, logits[:, -1], attns, line_scores):
        ...
        if int(example.label) == 1 and int(pred) == 1 and '[ADD]' in example.input_tokens:
            ...
            cur_IFA, cur_top_20_percent_LOC_recall, cur_effort_at_20_percent_LOC_recall, cur_top_10_acc, cur_top_5_acc = deal_with_attns(example,
                                                                                                                                         attn,
                                                                                                                                         pred,
                                                                                                                                         cur_codes,
                                                                                                                                         cur_labels,
...
def main(args):
    ...
    if args.do_train:
        ...
        if args.manual_checkpoint:
            ...
            checkpoint = torch.load(output_dir)
            model.load_state_dict(checkpoint['model_state_dict'], strict=False)
        train_dataset = TextDataset(tokenizer, args, file_path=args.train_data_file, buggy_lines_file_path=args.buggy_lines_file)
    ...
    if args.do_test:
        ...
        model.to(args.device)
        test_dataset = TextDataset(tokenizer, args, file_path=args.test_data_file, mode='test', buggy_lines_file_path=args.buggy_lines_file)
